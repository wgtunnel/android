...
sealed class RootShellException(override val message: String) : Exception(message) {
    ...
    class ShellStartFailed(exitCode: Int? = null, cause: Throwable? = null) :
        RootShellException(
            message =
                "Failed to start root shell${exitCode?.let { " (exit code: $it)" } ?: "${cause?.message}"}"
        )

    class DirectoryCreationFailed(val directory: String) :
