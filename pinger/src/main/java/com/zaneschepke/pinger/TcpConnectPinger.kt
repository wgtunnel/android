...
class TcpConnectPinger : Pinger {
    ...
    override suspend fun ping(config: PingConfig): PingStats = withContext(Dispatchers.IO) {
        ...
        val loss = if (transmitted == 0) 0.0 else ((transmitted - received) * 100.0 / transmitted)
    ...
    private fun createSocket(config: PingConfig): Socket {
        return when {
            config.proxy?.type() == Proxy.Type.SOCKS &&
                    config.proxyUsername != null && config.proxyPassword != null -> {
